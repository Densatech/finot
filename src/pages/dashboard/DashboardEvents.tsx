      ...
      {filteredEvents.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredEvents.map((event) => {
            ...
            return (
              <Card key={event.event_id} hoverable className={`overflow-hidden ${isPast ? "opacity-75" : ""}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {event.quote && (
                      <div className="mt-2 flex gap-2">
                        <FiMessageCircle className="h-4 w-4 flex-shrink-0 text-muted-foreground" />
                        <p className="text-sm italic text-muted-foreground line-clamp-2">{event.quote}</p>
                      </div>
                    )}
